...
function OnboardingStep({ number, title, description, icon: Icon }: {
  ...
  return (
    <div className="relative flex flex-col items-center">
      {number !== 3 && (
        <div className="hidden md:block absolute top-12 left-1/2 h-24 w-px bg-gradient-to-b from-orange-500 to-orange-300 md:w-full md:h-px md:top-12 md:-right-1/2 md:left-auto" />
      )}
...
function ContactForm() {
  ...
  const inputClasses = "w-full min-h-[48px] px-4 py-2 border border-gray-300 rounded-xl text-left text-base focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all";

  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-4 sm:p-8 max-w-2xl mx-auto w-full">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
        ...
        <div className="w-full">
          <label htmlFor="schoolName" className="block text-sm font-medium text-gray-700 mb-1 text-left">
            School Name *
          </label>
          <input
            type="text"
        ...
        <div className="w-full">
          <label htmlFor="studentsCount" className="block text-sm font-medium text-gray-700 mb-1 text-left">
            Number of Students
          </label>
          <select
            id="studentsCount"
      ...
      <div className="mt-6">
        <button
          type="submit"
          className="w-full md:w-auto px-8 py-4 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all flex items-center justify-center"
        >
          Schedule Your Demo
...
const Form: React.FC<FormProps> = () => {
  return (
    <div id="contact-form" className="min-h-screen">
